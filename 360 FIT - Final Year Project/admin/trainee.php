...
<head>
    ...
    if(isset($_GET['delete'])){
        ...
        $result = mysqli_query($conn,"DELETE FROM `gymplan` WHERE `id`='$id'");
        if($result){
            echo "<script language='javascript'>";
            echo 'alert("Deleted Successfully");';
            echo 'window.location.replace("trainee.php");';
            echo "</script>";
        }else{
            echo "<script language='javascript'>";
    ...
    if(isset($_GET['accept'])){
        ...
        $status="Accepted";
        $conn = mysqli_connect('localhost','root','','360_fit') or die('unable to connect');
        $result = mysqli_query($conn,"UPDATE `gymplan` SET `status`='$status' WHERE `id`='$id'");
        if($result){
            echo "<script language='javascript'>";
    ...
    if(isset($_POST['submit'])){
        ...
        $to_email = $email;
        $subject = "Zoom Link";
        $body = "Hi, $name this is the zoom link to your Gym appointment: $message";
